...
**✅ 可用**
- **岛上权限批准** —— Claude Code 的权限确认浮到岛上,点「允许 / 总是允许 / 拒绝」代替去终端按 y(基于官方 PermissionRequest hook)
- **AskUserQuestion 就地作答** —— 单选点选即提交、多选勾选 + 提交,答案直接回灌给会话,不用切回终端
- **多会话监控** —— 同时识别多个 Claude Code / Codex 会话;以转录文件路径为唯一标识 + 进程计数配对,**同一项目开多个会话也不会混**
- **会话详情** —— 最近消息流、当前动作、状态(运行中 / 在提问 / 已回复)
- **项目看板查看** —— 会话行的 📊 打开该项目的 `.board/board.html`(看板内容由外部 `/update-board` 技能生成)
- **打包 DMG** —— `build_app.sh` / `package_dmg.sh`

**⚠️ 部分可用 / 有已知边界**
- **终端精确跳转**:Terminal.app / iTerm2 可精确定位标签;Ghostty / Warp / VSCode 只能激活 App(终端本身无定位接口)
- **Codex 元数据**:目前是"二等公民"——能识别会话与活动时间,但没有 pendingQuestion / 当前动作 / 最近消息(这些仅 Claude Code 有)
...
目前从源码构建(分发用的 DMG 尚未做 Apple 公证,Homebrew cask 在路线图上):
...
```bash
./scripts/install_hooks.sh   # 安装 hook 到 ~/.devisland/,并把配置写入 ~/.claude/settings.json(自动备份)
```
...
```
Sources/DevIsland/
├── DevIslandApp.swift          # 入口:MenuBarExtra 菜单栏场景
